module.exports = async (sock, sender, text, msg) => {
    try {
        ...
        if (mentionedUsers.length === 0) {
            await sock.sendMessage(groupId, { text: '❌ Please mention the user(s) you want to demote. ☘️Ⓜ️' });
            return;
        }
        ...
        if (!groupAdmins.includes(botNumber)) {
            await sock.sendMessage(groupId, { text: '❌ I need to be an admin to demote users. ☘️Ⓜ️' });
            return;
        }
        ...
        for (const user of mentionedUsers) {
            if (!groupAdmins.includes(user)) {
                await sock.sendMessage(groupId, { 
                    text: `❌ User @${user.split('@')[0]} is not an admin. ☘️Ⓜ️`, 
                    mentions: [user] 
                });
                continue;
            }
            ...
            try {
                ...
                await sock.sendMessage(groupId, { 
                    text: `✅ User @${user.split('@')[0]} has been demoted from admin. ☘️Ⓜ️`, 
                    mentions: [user] 
                });
            } catch (error) {
                console.error(`Error demoting user ${user}:`, error);
                await sock.sendMessage(groupId, { 
                    text: `❌ Failed to demote @${user.split('@')[0]}: ${error.message} ☘️Ⓜ️`, 
                    mentions: [user] 
                });
            }
        }
        ...
        await sock.sendMessage(sender, { text: '❌ Failed to process the demote command. Please try again. ☘️Ⓜ️' });
    }
};